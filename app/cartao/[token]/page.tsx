...
export default async function CartaoPage({ params }: PageProps) {
  ...
  const local = evento.nome_local || evento.local || "";
  const endereco = evento.endereco || [evento.rua, evento.numero, evento.bairro, evento.cidade].filter(Boolean).join(", ");
  const logoUrl = evento.logo_url || evento.logo_image || "";
  const backgroundUrl = evento.background_url || evento.background_image || "";

  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=900x900&data=${encodeURIComponent(token)}`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p style={{ color: "#d7b56d", fontSize: 14 }}>
        ...
        >
          ...
          >
            Guarde este cartão no seu WhatsApp e apresente o QR Code no totem de
            recepção para agilizar seu check-in.
          </p>
